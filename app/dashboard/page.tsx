...
export default async function DashboardOverview() {
  ...
  if (!data.success && data.reason === "missing-user") {
	return (
	  <div className="p-8">
		...
		<p className="text-muted-foreground">
		  We couldn't find an application user for this account. Please contact
		  support.
		</p>
	  </div>
	);
  ...
  if (!data.success) {
	return (
	  <div className="p-8">
		...
		<p className="text-muted-foreground">
		  Couldn't fetch projects and documents, please try again later.
		</p>
	  </div>
	);
  }
  ...
  return (
	<div className="flex flex-col gap-12">
	  ...
	  />
	  <ShowDocuments recentDocuments={visibleDocuments} page={"overview"} />
	</div>
  );
}
