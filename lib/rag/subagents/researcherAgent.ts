  ...
  async ({ query, projectId }: { query: string; projectId: string }) => {
	try {
	  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
		...
		console.log(
		  `researcherAgent: attempt ${attempt + 1}/${MAX_ATTEMPTS}${isRetry ? " (retry)" : ""}`,
		);

		// 1. Expand query into 3 diverse search queries
		const multiQueryResp = await multiQueryAgent.invoke({
		  query,
		  retry: isRetry || undefined,
		});
		console.log(
		  "researcherAgent: multiQuery response:",
		  JSON.stringify(multiQueryResp),
		);
		if (!multiQueryResp.success || !multiQueryResp.data?.length) {
		  console.log(
			"researcherAgent: multiQuery FAILED, returning NO_RELEVANT_DATA_FOUND",
		  );
		...
		const hybridResp = await hybridSearch({
		  queries: allQueries,
		  projectId,
		});
		console.log(
		  "researcherAgent: hybridSearch response success:",
		  hybridResp.success,
		  "count:",
		  hybridResp.data?.length ?? 0,
		);
		if (!hybridResp.success || !hybridResp.data?.length) {
		  console.log(
			"researcherAgent: hybridSearch FAILED:",
			hybridResp.message,
		  );
		  return "NO_RELEVANT_DATA_FOUND";
		}
		...
		console.log(
		  "researcherAgent: rerank response success:",
		  rerankResp.success,
		  "count:",
		  rerankResp.data?.chunks?.length ?? 0,
		);
		...
		console.log(
		  "researcherAgent: reranked",
		  rerankedChunks.length,
		  "chunks",
		);

		// 3. Grade each chunk
		const gradeResp = await graderAgent.invoke({
		...
		const validChunks = rerankedChunks.filter((c) =>
